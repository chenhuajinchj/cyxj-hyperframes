...
### 真实素材清单（2026-05-22 落实后状态）

| 文件 | 用途 | 状态 |
|---|---|---|
| `assets/tweets/joining-tweet.png` | SEC A 主截图 - Karpathy 加入推文 | ✅ 已就位 |
| Joseph 推文 → **走引文 panel** | SEC A hot moment | ⚠ 改路径：X.com / nitter / Forbes / Observer / mlq / TheNewStack / Substack 全部抓不到 embed（X 反爬 + Substack paywall + 文章站只引文字）。改用「文字引用 panel + source badge `x.com · @nickevanjoseph · 2026-05-19`」+ Joseph 头部 chip。**这是规范的二次引用**，跟 [[feedback-real-screenshots-not-mocks]] 不冲突——Joseph 原文已 grok 双源 + The New Stack + mlq.ai + Observer 文字一致核对 |
...
| `assets/portraits/dario.jpg` | SEC E ex-OpenAI 头像之一（CEO） | ✅ 已就位 + 已核对真人 |
| `assets/portraits/daniela.png` | SEC E ex-OpenAI 头像之一（President）| ✅ Wired 报道页 crop 真人确认（棕波浪发 + 黑红 blazer + 项链） |
| `assets/portraits/jaredkaplan.jpg` | SEC E ex-OpenAI 头像之一（GPT-3 主作者）| ✅ 已就位 + 已核对真人（TC AI Sessions backdrop）|
| `assets/portraits/tombrown.png` | SEC E ex-OpenAI 头像之一（GPT-3 lead）| ✅ Business Insider 报道页 crop 真人确认（短发笑容 + 暖灰背景） |
...
| "A 社预训练负责人 Joseph 推特的原话是" | A | Karpathy 头像滑到左侧 1/3，右侧 2/3 弹入**Karpathy 加入推文真截图卡**（joining-tweet.png）+ 下方副卡**「Joseph 引文 panel」**：暗米色边框 + Joseph 用户名 chip + 引文文本（英文原句）+ source badge `x.com · @nickevanjoseph · 2026-05-19`。**引文不是 mock**——是多源核对过的真原文（grok 双源 + The New Stack + mlq.ai + Observer 一致），符合规范二次引用 |
| "用 Claude 加速预训练研究本身 / 用 Claude 去训练 Claude" | A | Joseph 引文 panel 中"**using Claude to accelerate pretraining research itself**"那段用 Claude 橙 marker sweep 高亮 → 引文 panel 淡出 → 屏幕中央出现一个**循环箭头**：左 `Claude` 节点 → 右 `Claude.next` 节点 → 回环。这是 SEC A 的 hot moment（小幅，非 chrome 大字） |
...
- 12-18s：Karpathy 头像滑到左侧 1/3（x: from 0 to -300px in 1s），右侧 2/3 上半弹入 **joining-tweet.png 真截图卡**（spring ease back.out(1.5)，scale 0.95→1）
- 18-22s：joining-tweet 下方弹入 **Joseph 引文 panel**（米色暖底卡 + 1px Claude 橙边框 + 顶部 chip `@nickevanjoseph` 灰圆头像 + Anthropic 蓝标 + 引文英文原句 token-stagger 0.06s + 底部 source badge `x.com · 2026-05-19`）
- 22-26s：引文中 "**using Claude to accelerate pretraining research itself**" 关键句 marker sweep 高亮（Claude 橙 background-position animate from -100% to 0%，1.4s）—— **本段 hot moment**
- 26-29.5s：joining-tweet + Joseph 引文 panel 同步淡出，屏幕中央出现**循环箭头**：`Claude → Claude.next → loop back`。hold 1s 后 SEC 末

**关键素材**：karpathy-hero.png + joining-tweet.png + Joseph 引文（HTML panel 渲染引文真原文 + 真 source badge）
