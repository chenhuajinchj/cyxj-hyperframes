...
## 字幕对位审计（2026-05-21 v3）

工程已落地（commit `e6e2f21` v2），现在拿字幕条 1-11（真源 `字幕/加入之后.srt`）逐句对照实际工程（`compositions/ch0-cold-open.html`）的视觉设计。

### 字幕条 1-11 全文

| # | 时间 | 字幕原文 |
|---|---|---|
| 1 | 0:00.133-5.633 | OpenAI的创始成员卡帕西加入了OpenAI最大的竞争对手Anthropic |
| 2 | 0:06.133-7.833 | 这个不是一个跳槽八卦 |
| 3 | 0:08.199-10.500 | 背后是过去两年AI行业最大的一次转向 |
| 4 | 0:10.500-12.833 | 转向只是大多数人还没有意识到而已 |
| 5 | 0:13.500-16.033 | 为什么我觉得这件事不是普通的跳槽 |
| 6 | 0:16.133-20.533 | 因为上个月前Anthropic在美国企业市场首次反超了OpenAI |
| 7 | 0:20.733-27.033 | 34.44%对32.3%，三年前Anthropic的份额才只有0.03% |
| 8 | 0:27.333-30.399 | 一个OpenAI的创始成员在这一个时间 |
| 9 | 0:30.399-33.633 | 点去了OpenAI竞争对手的一家模型公司 |
| 10 | 0:33.633-35.733 | 为什么是这一个时间点以及 |
| 11 | 0:36.366-37.666 | 为什么是Anthropic |

### 逐 SEC 审计结果

#### SEC A 迁徙 (0-5.6s · 字幕条 1)

| 元素 | 来源 | 决策 |
|---|---|---|
| 推文 banner（`图片用户收集/加入的推文.png`） | ⚠️ 非字幕：字幕没提"推文" | **保留** — 视觉证据，把口播"加入了 Anthropic"具象化（用户原图） |
| OpenAI logo + chip "创始成员 · 2015" | ✅ 字幕条 1 "OpenAI的创始成员" | 保留 |
| Karpathy 头像（`assets/portraits/karpathy-hero.png`） | ⚠️ 非字幕：字幕没说"卡帕西长什么样" | **保留** — 人物视频核心元素，常识性视觉补强 |
| 虚线箭头 + 头像沿虚线推 | ✅ 字幕条 1 "加入了…对手" = 物理迁徙 | 保留 |
| Anthropic logo + chip "2026.5.19 加入" | ✅ 字幕条 1 "加入了…Anthropic" | 保留 |
| 底部 caption "OpenAI 的 创始成员 · 加入了 OpenAI 最大的 竞争对手" | ✅ 字幕条 1 几乎直接复述 | 保留 |

#### SEC B 否定+转向 (5.6-12.8s · 字幕条 2-4)

| 元素 | 来源 | 决策 |
|---|---|---|
| "跳槽八卦" chip + 划掉斜杠 | ✅ 字幕条 2 "这个不是一个跳槽八卦" | 保留 |
| chip 缩到角落 | ✅ 否定后退场视觉化 | 保留 |
| chrome 大字"AI 行业最大的一次**转向**" | ✅ 字幕条 3 直接 | 保留 |
| 副字"只是大多数人还没意识到"（char stagger） | ✅ 字幕条 4 直接 | 保留 |

#### SEC C 自问 (12.8-16.1s · 字幕条 5)

| 元素 | 来源 | 决策 |
|---|---|---|
| chrome 大字"为什么这不是普通的跳槽？" | ✅ 字幕条 5 "为什么我觉得这件事不是普通的跳槽"（口语化简化，去掉"我觉得"是合理减字幅） | 保留 |
| "？" elastic 放大 | ✅ 字幕是问句，视觉化问号合理 | 保留 |
| 右下角 hint "答案在数据里 ↓" | ⚠️ 非字幕：字幕没提"答案在数据里" | **保留** — 作为视觉过渡 cue 引导视线到 SEC D（数据反超）|

#### SEC D 数据反超 (16.1-27.3s · 字幕条 6-7)

| 元素 | 来源 | 决策 |
|---|---|---|
| 标题"Anthropic 在美国企业市场首次反超 OpenAI" | ✅ 字幕条 6 几乎直接复述 | 保留 |
| 英文副标题 "U.S. Enterprise AI Adoption · Monthly Share" | ⚠️ 非字幕 | **保留** — 数据可视化的专业感标注（来自 Ramp 报告） |
| OpenAI 折线（高位稳定） | ✅ 字幕条 7 隐含（"32.3%"是单点，但要画出"反超前的稳定"才能让"反超"成立） | 保留 |
| Anthropic 折线（陡升从 0.03 到 34.44） | ✅ 字幕条 7 "三年前 0.03% / 现在 34.44%" 直接来源 | 保留 |
| X 轴时间刻度 2023.6 / 2024.6 / 2025.4 / 2025.10 / 2026.5 | ⚠️ 字幕只有"上个月"和"三年前"（2 个点），**2024.6 / 2025.4 / 2025.10 是研究/03 的中间点** | **保留** — 视觉平滑度，让"渐升曲线"成立。事实来源：`研究/03-anthropic-events.md` |
| 左上角"三年前对比"数据（0.003% / 7.94% / 34.44%） | ⚠️ 2025.4 = 7.94% 字幕没有 | **保留** — 呼应 X 轴 2025.4 节点，给"渐升"曲线视觉支撑（事实来源：研究/03） |
| 反超撞击 §3 模板（ripple + screen shake + glow + badge） | ✅ "首次反超"是字幕条 6 强论点 | 保留 |
| "↑ 首次反超" badge | ✅ 字幕条 6 "首次反超"直接 | 保留 |
| 末端 34.44% / 32.3% label | ✅ 字幕条 7 直接 | 保留 |
| 底部 source "Source: Ramp 企业 AI 采纳指数 · 5 万 + 美国企业真实消费" | ⚠️ 字幕没说 Ramp 来源 + 没说 5 万家 | **保留** — 事实佐证可信度（来源：研究/03，Ramp Lead Economist Ara Kharazian 报告）|

#### SEC E 三连问 (27.3-38s · 字幕条 8-11)

| 元素 | 来源 | 决策 |
|---|---|---|
| chip 1 "OpenAI 创始成员" | ✅ 字幕条 8 "一个 OpenAI 的创始成员" | 保留 |
| chip 2 "这个时间点" | ✅ 字幕条 8 "在这一个时间" | 保留 |
| chip 3 "Anthropic 对手公司" | ✅ 字幕条 9 "OpenAI 竞争对手的一家模型公司" + 字幕条 11 "Anthropic" | 保留 |
| ~~bridge-line "在 OpenAI 被反超的同一个月，去了反超它的那家"~~ | ❌ **凭空加的小陈视角措辞，字幕里没有** | **剔除** ✂️（已删 CSS / HTML / JS 三处）|
| chrome 大字 q1 "为什么是这个时间点？" | ✅ 字幕条 10 "为什么是这一个时间点" | 保留 |
| chrome 大字 q2 "为什么是 Anthropic？" | ✅ 字幕条 11 "为什么是 Anthropic" | 保留 |
| 末尾 fade out | ✅ 章末（N18 例外）| 保留 |

### 审计总结

- **剔除 1 项**：SEC E 中间衔接小字 "在 OpenAI 被反超的同一个月，去了反超它的那家"（凭空加 + 无保留理由）
- **保留+标注 8 项**：推文 banner / Karpathy 头像 / SEC C hint / SEC D 副标题 / X 轴中间刻度 / 三年前对比 / Ramp source —— 均为视觉证据或事实佐证，事实正确（研究文档有）
- **完全字幕直接 10+ 项**：5 SEC 的核心 chrome 大字 + chip + 折线 + caption 全部字幕来源

### 验收

- 改动后 SEC E DOM 少一个 `.bridge-line` 元素
- timeline 少一个 tween（27.3 + 4.2 那个 fromTo 已删）
- 字幕条 8-9 (27.3-33.6s) 节奏：3 chip stagger 入场（间隔 1.0-1.5s），到字幕条 10 (33.6s) chrome 大字砸下时 SEC E 干净留白
- 跑 lint + validate 必须仍 0 errors / 0 contrast warnings
- thumbnail API 截 t=30 应该没 bridge-line 小字，只有 3 chip
- thumbnail API 截 t=36 应该跟之前一样（q1 显示）
