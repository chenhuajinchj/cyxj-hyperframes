...
> ⚠️ **Codex 镜像滞后说明（2026-05-06）**：
> 本 skill 落后 `.claude/skills/cyxj-new-video/SKILL.md` 数轮迭代——缺 A0 URL 分流 / A0.5 决策树 / `tutorial-8beat` 选项 / 阶段 D 风格借鉴 / 硬约束单源指向。**A1 第 1 题中 A/B/C 选项均已废弃**（伪模板）。
...
1. **视频形态**（影响选哪个模板）：
   - A. ⚠️ **已废弃** 录屏出镜 → `templates/host-overlay/`（伪模板：claude-demo-v2 旧工程 rename，请从 0 写）
   - B. ⚠️ **已废弃** 录屏要在达芬奇精修 → `templates/host-overlay-alpha/`（伪模板：claude-overlays-only 旧工程 rename，请从 0 写）
   - C. ⚠️ **已废弃** 纯演示无录屏 → `templates/demo-fullscreen/`（伪模板：codex-claude-intro 旧工程 rename，请从 0 写）
